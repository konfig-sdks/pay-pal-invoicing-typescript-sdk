...
import { readableStreamToString, PayPalInvoicingError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PayPalInvoicingError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new PayPalInvoicingError(e, e.response?.data)
                }
                if (innerError instanceof PayPalInvoicingError) {
                    // Got "PayPalInvoicingError" from the above try block
                    throw innerError;
                }
